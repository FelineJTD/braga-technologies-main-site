  ...
  return (
    <section ref={sectorsRef} className='whiteBGFullGrid'>
      ...
      <div className='col-span-6 md:col-start-3 md:col-span-2 lg:col-start-5 lg:col-span-4'>
        <h6 className='text-gray-800 text-[20px] lg:text-[22px] mb-6'>
          We have navigated enterprises and organizations across these sectors.
        </h6>
        ...
        {Sectors.map((sector, index) => (
          <div key={index} className='flex flex-col space-y-1 relative z-0'>
            <button
              key={index}
              className={`${
                selectedSectorIdx === index
                  ? "buttonSelectionTimedSelected"
                  : "buttonSelection"
              } font-normal relative min-w-[8rem] !flex`}
              onClick={() => setSelectedSectorIdx(index)}
            >
              <img src={sector.icon} alt='' width={16} height={16} />
              <h3 className='text-left mt-4 text-gray-800 ml-3 text-2xl md:text-xl lg:text-3xl '>
                {sector.title}
              </h3>
            </button>
          </div>
