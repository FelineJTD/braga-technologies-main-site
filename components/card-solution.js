...
export default function CardSolution({ solution, isDarkMode = false }) {
  return (
    <div
      className={`group snap-start lg:snap-none flex flex-col space-y-2 w-full rounded-lg p-3 border-2 ${
        isDarkMode ? "border-gray-600" : "border-gray-200"
      }`}
    >
      <div
        className={`${
          solution.animation ? "bg-gray-100" : "aspect-[287/160]"
      ...
      <div className='flex justify-between justify-self-end items-center space-x-3'>
        <p
          ...
          } whitespace-nowrap rounded-3xl py-2 px-3 text-xs`}
        >
          {solution.type}
        </p>
        {!solution.comingSoon && (
          ...
          >
            <button
              className={`border-[1px] ${
                isDarkMode
                  ? "border-white enabled:hover:bg-white enabled:hover:text-black"
                  : "border-black enabled:hover:bg-black enabled:hover:text-white"
              } disabled:opacity-30 duration-300 whitespace-nowrap`}
            >
              Visit &nbsp;&nbsp; &gt;
            </button>
          </a>
