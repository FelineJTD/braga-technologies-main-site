...
export default function CardSector({sector, selectedSectorTitle, index, isMobile=false}) {
  return (
    <div className={`${isMobile ? 'self-center mb-12' : (index%2 == 0 ? 'mt-8' : '-mt-12 self-end')} ${isMobile ? (sector.title == selectedSectorTitle ? 'opacity-100 relative' : 'opacity-0 absolute') : (sector.title == selectedSectorTitle ? 'opacity-100' : 'opacity-0 translate-y-3')} w-2/3 rounded-lg border-[1px] border-gray-200 duration-300`}>
